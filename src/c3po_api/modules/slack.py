...
from c3po_api.modules import scoring
...
class SlackEventHandler(object):
    ...
    def mention_parser(self, request):
        ...
        try:
            if 'leaderboard' in request['event']['text']:
                app.logger.info('Leaderboard requested')
                message['text'] = scoring.get_leaderboard()

            elif 'my score' in request['event']['text']:
                app.logger.info('My score requested')
                user = scoring.get_user_score(slack_id=request['event']['user'])
                message['text'] = f"<@{request['event']['user']}>\'s score is {str(user.score)} and you have {str(user.bank)} left to give today"

            elif 'user score' in request['event']['text']:
                app.logger.info('User score requested')
                regex = '<@.{8,13}>'
                slack_ids = re.findall(regex, str(request['event']['text']))
                user = scoring.get_user_score(slack_id=slack_ids[1])
                message['text'] = f"<@{user.slack_id}>\'s score is {str[user.score]}"
            
            elif 'help me' in request['event']['text']:
                ...
                message['text'] = f"Sorry. I didn't understand what you said.\n\nFor help, type `@{bot_info['bot']['name']} help me`"
